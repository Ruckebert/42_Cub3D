...
static void compute_wall_dimensions(t_game *game, double corrected_dist, int *start, int *end)
{
    double proj_plane_dist = (game->win_x / 2.0) / tan(FOV / 2.0);
    int slice_height = (int)(proj_plane_dist / corrected_dist);
    
    *start = (game->win_y / 2) - (slice_height / 2);
...
{
    int y = params->start;
    int wall_height = params->end - params->start;
    
    while (y <= params->end)
    {
        int tex_y_offset = (y * 256) - (game->win_y * 128) + (wall_height * 128);
        int tex_y = ((tex_y_offset * params->tex->height) / wall_height) / 256;
...
// Simplified ray drawing for debugging/minimap
